...
public class FontRenderer extends TextureWrapper {
    ...
    public void drawStringWithShadow(@NotNull String string, float x, float y, float r, float g, float b, float a) {
        ...
        {
            int color = (((int) (a * 255 + 0.5) & 0xFF) << 24) | // To convert from normalized 0-1 SOUND_RESOURCES efficiently -> ceil(val * 255) is optimized to int(val * 255 + 0.5). Adding +0.5 ceils if cast to an int.
     ...
     * @param b         blue color component
     * @param a         alpha (transparency) component
     * @return true if the char was rendered (or skipped because it is a white character). If the character was not rendered because it is not supported, the function returns false
     */
    private boolean drawChar(float x, float y, char character, float r, float g, float b, float a) {
        if (character == ' ') {
            return true;
        }
