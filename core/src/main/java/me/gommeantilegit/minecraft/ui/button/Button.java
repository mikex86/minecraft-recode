...
public abstract class Button extends Renderable {
    ...
    public void receiveMouseInput(float mouseX, float mouseY,
        ...
        switch (action) {
            case 0:
                if (buttonID == 0)
                    if (isHovered(mouseX, mouseY) && pointer == -1) {
            ...
            case 1:
                if (buttonID == 0) {
                    if (pressed && clickPointer == pointer) {
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        receiveMouseInput(screenX, screenY, button, 0, pointer);
        if (!this.draggedInside && this.dragInsidePointer == -1 && button == 0 && isHovered(screenX, screenY)) {
            onDragEnter(pointer);
        }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        receiveMouseInput(screenX, screenY, button, 1, pointer);
        if (this.draggedInside && this.dragInsidePointer == pointer && button == 0) {
            onDragExit(pointer);
        }
    ...
    public boolean touchDragged(int screenX, int screenY, int pointer) {
        if (this.dragInsidePointer == -1 && !draggedInside && isHovered(screenX, screenY)) {
            onDragEnter(pointer);
        }
        if (dragInsidePointer != -1 && draggedInside && pointer == this.dragInsidePointer && !isHovered(screenX, screenY)) {
            onDragExit(pointer);
        }
    ...
    private void onDragExit(int pointer) {
        ...
        if (this.onDragLeaveListener != null)
    ...
    private void onDragEnter(int pointer) {
        ...
        if (this.onDragEnterListener != null)
    ...
    public void setOnMouseDownListener(@Nullable OnMouseDownListener onMouseDownListener) {
